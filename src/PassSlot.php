...
class PassSlot
{
     ...
     * Default user agent
     */
    const USER_AGENT = 'PassSlotSDK-PHP/0.6.1';
    ...
    {
        $resource = sprintf("/passes/%s/%s", $passTypeIdentifier, $serialNumber);
        return $this->_restCall('GET', $resource);
    }
     ...
     *
     */
    public function getPassJson($passTypeIdentifier, $serialNumber )
    {
        $resource = sprintf("/passes/%s/%s/passjson", $passTypeIdentifier, $serialNumber);
        return $this->_restCall('GET', $resource);
    }
     ...
     * @throws PassSlotApiException API Exception
     */
    public function getPassValues($passTypeIdentifier, $serialNumber)
    {
        $resource = sprintf("/passes/%s/%s/values", $passTypeIdentifier, $serialNumber);
        return $this->_restCall('GET', $resource);
    }
    ...
    {
        $resource = sprintf("/passes/pass.loyalty");
        return $this->_restCall('POST', $resource);
    }
     ...
     * @throws PassSlotApiException API Exception
     */
    public function getPassURL($passTypeIdentifier, $serialNumber)
    {
        if ($pass->url) {
            return $pass->url;
        }
        $resource = sprintf("/passes/%s/%s/url", $passTypeIdentifier,$serialNumber);
     ...
     * @throws PassSlotApiException API Exception
     */
    public function updatePassValue($passTypeIdentifier, $placeholderName, $serialNumber,$value)
    {
        $resource = sprintf("/passes/%s/%s/values/%s", $passTypeIdentifier, $serialNumber, $placeholderName);
